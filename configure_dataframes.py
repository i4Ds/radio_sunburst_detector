...
def directory_to_dataframe(label, dataframe=None ):
    ...
    if dataframe == None:        
        ...
        new_df = pd.DataFrame({'time_period': start_times, 'file_path': file_paths, 'label': labels})
...
if __name__ == "__main__":
    ...
    configured_burst_df = configure_data_frame(burst_df, IMAGE_NUM_PER_LABEL) #sorted=False if you do not want sorting
    configured_noburst_df = configure_data_frame(noburst_df, IMAGE_NUM_PER_LABEL) #sorted=False if you do not want sorting
